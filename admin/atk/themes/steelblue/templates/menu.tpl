    ...
    <div id="smi_{$menuitem.name}" class="submenuHover">
      ...
      {foreach from=$menuitem.submenu item=submenuitem}
         {if $submenuitem.enable && $submenuitem.name!=='-'}
           <a class="menuItemLevel2" onclick="window.open('{$submenuitem.url}','main','');myfun(this);" onmouseover="this.style.cursor = 'pointer'; this.style.color = '#9a1010';" onmouseout="this.style.color = '#414141';">
             {$submenuitem.name}
           </a>
         {/if}
      {/foreach}
...
{
if(arr[i].className=='myclass')
{
arr[i].className='menuItemLevel2';
}
