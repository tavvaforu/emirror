...
$nn=$memar['first_name'];
$tmail=$memar['email'];
$tpl_object = new Template("templates/journal");
//$tpl_object = new Template(TEMP_ROOT."journal");
...
//echo "select a.*,b.message_intensity as color_id from messagetrigger a,message_intensity b where a.message_color_id=b.id and a.id='$id'";
  ...
  $title_journal=$my_arrdraft['title'];
...
$r1=mysql_query("select id from messagetrigger where  sender_email='$tmail' and message_type_id=2 and send_type=0  and status!='SDeleted' and status!='Deleted' ");
$tot_sent=mysql_num_rows($r1);
//echo "select id from messagetrigger where  email='$tmail' and status!='RDeleted' and status!='Deleted' ";
$r2=mysql_query("select id from messagetrigger where  email='$tmail' and message_type_id=2 and send_type=0 and status!='RDeleted' and status!='Deleted' ");
...
{
	$ch1=mysql_query("SELECT message_color_id FROM messagetrigger  where  sender_email='$tmail' and message_type_id=2 and status!='SDeleted' and send_type =0 and status!='Deleted' and message_color_id=".$my_colors['id']);
...
while($my_colors1=mysql_fetch_array($colorsQuery1))
{
	...
	$ch1=mysql_query("SELECT message_color_id FROM messagetrigger  where  email='$tmail' and message_type_id=2 and send_type=0 and status!='SDeleted' and status!='Deleted' and message_color_id=".$my_colors1['id']);
...
//Insertion of journals

if($_POST['saveForm']=="Save")
{
//echo '<PRE>';print_r($_POST);exit;
	$title=$_POST['title_journal'];
	...
	if($insid)
	  header("location:index.php?file=j-journal&msg=$msg&stype=$saveType#page=page-2");
	?>
	...
	if($_POST['saveForm']=="Delete")
{
  ...
  header("location:index.php?file=j-journal&stype=$stype&msg=$msg#page=page-2");
}
...
$my_res1=mysql_query("select a.*,b.message_intensity as color_id from messagetrigger a,message_intensity b where a.message_color_id=b.id and  message_type_id='2' and sender_email='$tmail' and send_type=0 $msql $dsql and status!='Deleted' and status!='SDeleted'");
//}
/*paging statrs */
...
$reload = $_SERVER['PHP_SELF'] . "?file=j-journal&stype=".$stype."&tpages=" . $tpages . "&amp;adjacents=" . $adjacents;
...
$my_res=mysql_query("select a.*,b.message_intensity as color_id from messagetrigger a,message_intensity b where a.message_color_id=b.id and message_type_id='2' and email='$tmail' $msql $dsql and status!='Deleted' and status!='RDeleted' order by $order $ot LIMIT $start,$limit");
}
else
{*/
...
$my_res=mysql_query("select a.*,b.message_intensity as color_id from messagetrigger a,message_intensity b where a.message_color_id=b.id and  message_type_id='2' and sender_email='$tmail' and send_type=0 $msql $dsql and status!='Deleted' and status!='SDeleted' order by $order $ot LIMIT $start,$limit");
//}
...
$res.='	<span style="float:left;margin:0px 0px 5px 0px; padding:0px; width:150px;">
 ...
 $res.='<img src="{IMG_URL}attachemtn.gif" width="16" height="14" alt="attachemet" />';
  ...
  if($send_type==0) { 
  $res.='<a href="index.php?file=j-open_journal&stype='.$send_type.'&id='.$id.'">'.$message.'</a></label>';
   } else{
  $res.='<a href="index.php?file=j-open_journal&stype='.$send_type.'&id='.$id.'">'.$message.'</a>';
  }
$res.='</label>
</span>
...
$res.='<a href="#" onclick="streamPublish(\''.$my_arr['title'].'\',\''.$message.'....\',\'\');" ><img src="{IMG_URL}facebook_icon.gif" width="16" height="16" border="0"/></a>
  ...
  <a href="#" onclick="return checkjournalDelete('.$id.')";><img src="{IMG_URL}delete.gif" width="13" flot="right"height="13" /></a></div>
</div>';
...
//echo $order;
$send_type=0;
$res1='<a href="index.php?file=j-journal&stype='.$send_type.'#page=page-2">All</a>&nbsp;&nbsp;&nbsp;';
$res1.= $pagerecords.paginate_one($reload,$page,$tpages);
if($order=="message" && $otype=="a")
{
$tp=str_replace("{MSG_SORT}",'<a href="index.php?file=j-journal&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order=message&otype=d#page=page-2" >
<img src="{IMG_URL}up_arrow.png" width="12" height="6" alt="descendingorder" title="Descending Sort" align="absmiddle"  border="0" /></a>',$tp);
}
else
{
$tp=str_replace("{MSG_SORT}",'<a href="index.php?file=j-journal&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order=message&otype=a#page=page-2" >
...
{
$tp=str_replace("{SENDER_SORT}",'<a href="index.php?file=j-journal&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order='.$osender.'&otype=d#page=page-2" >
<img src="{IMG_URL}up_arrow.png" width="12" height="6" alt="descendingorder" title="Descending Sort" align="absmiddle"  border="0" /></a>',$tp);
}
else
{
$tp=str_replace("{SENDER_SORT}",'<a href="index.php?file=j-journal&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order='.$osender.'&otype=a#page=page-2" >
<img src="{IMG_URL}down_arrow.png" width="12" height="6" alt="ascendingorder" title="Ascending Sort" align="absmiddle"  border="0" /></a>',$tp);
}
if($order=="create_time" && $otype=="a")
{
$tp=str_replace("{TIME_SORT}",'<a href="index.php?file=j-journal&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order=create_time&otype=d#page=page-2" >
<img src="{IMG_URL}up_arrow.png" width="12" height="6" alt="descendingorder" title="Descending Sort" align="absmiddle"  border="0" /></a>',$tp);
}
else
{
$tp=str_replace("{TIME_SORT}",'<a href="index.php?file=j-journal&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order=create_time&otype=a#page=page-2" >
<img src="{IMG_URL}down_arrow.png" width="12" height="6" alt="ascendingorder" title="Ascending Sort" align="absmiddle"  border="0" /></a>',$tp);
}
if($order=="message_intensity" && $otype=="a")
{
$tp=str_replace("{INTENSITY_SORT}",'<a href="index.php?file=j-journal&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order=message_intensity&otype=d#page=page-2" >
<img src="{IMG_URL}up_arrow.png" width="12" height="6" alt="descendingorder" title="Descending Sort" align="absmiddle"  border="0" /></a>',$tp);
}
else
{
$tp=str_replace("{INTENSITY_SORT}",'<a href="index.php?file=j-journal&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order=message_intensity&otype=a#page=page-2" >
<img src="{IMG_URL}down_arrow.png" width="12" height="6" alt="ascendingorder" title="Ascending Sort" align="absmiddle"  border="0" /></a>',$tp);
}

$tp=str_replace("{DRAFT_BTN}",'<input id="saveForm" name="saveForm" class="inr_btn" type="button" value="Preview" onclick="return mypopup(document.messageform);"/>
<input id="saveForm" onclick="return valid_journalform(document.messageform,\''.$str_words.'\');" name="saveForm" class="inr_btn" type="submit" value="Save"/>
   ',$tp);
...
/*$tp=str_replace("{RECEIVED}","<a href='journal.php?stype=2&order=$order#page=page-2' ".$classR .">Received</a>",$tp);
...
//journal messages
$tp=str_replace("{title}",$title_journal,$tp);
