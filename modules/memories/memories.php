...
$nn=$memar['first_name'];
$tmail=$memar['email'];
$tpl_object = new Template("templates/memories");
...
//echo "select a.*,b.message_intensity as color_id from messagetrigger a,message_intensity b where a.message_color_id=b.id and a.id='$id'";
  ...
  //echo '<pre>';print_r( $my_arrdraft);exit;
  $motion= strtolower($my_arrdraft['motion']);
  $id=$my_arrdraft['id'];
  $title_memory=$my_arrdraft['title'];
...
$r1=mysql_query("select id from messagetrigger where  sender_email='$tmail' and message_type_id=4 and send_type=0  and status!='SDeleted' and status!='Deleted' ");
$tot_sent=mysql_num_rows($r1);
//echo "select id from messagetrigger where  email='$tmail' and status!='RDeleted' and status!='Deleted' ";
$r2=mysql_query("select id from messagetrigger where  email='$tmail' and message_type_id=4 and send_type=0 and status!='RDeleted' and status!='Deleted' ");
...
{
	$ch1=mysql_query("SELECT message_color_id FROM messagetrigger  where  sender_email='$tmail' and message_type_id=4 and status!='SDeleted' and send_type =0 and status!='Deleted' and message_color_id=".$my_colors['id']);
...
while($my_colors1=mysql_fetch_array($colorsQuery1))
{
	...
	$ch1=mysql_query("SELECT message_color_id FROM messagetrigger  where  email='$tmail' and message_type_id=4 and send_type=0 and status!='SDeleted' and status!='Deleted' and message_color_id=".$my_colors1['id']);
...
//echo '<PRE>';print_r($_POST);exit;
	...
	//if($_POST['saveForm']=='Save'){
		$send_type = "0";
		$msg="Memories Saved succesfully.";
		$saveType = 0;
	/*} else {
	$send_type="1";
	...
	mysql_query("update messagetrigger set sender_name='".$nn."',sender_email='".$tmail."',title='".$title."',message='".$message."', message_intensity='".$message_intensity."',name='".$name."',email='".$email."',message_type_id='".$message_type_id."',send_type='".$send_type."',message_color_id='".$message_color_id."' where id='".$_POST['draftid']."'");
	$insid=$_POST['draftid'];
	}else{
	//echo 'insert';exit;
	mysql_query("insert into messagetrigger(sender_name,sender_email,message,message_intensity,name,email,message_type_id,send_type,title,message_color_id)values('$nn','$tmail','$message','$message_intensity','$name','$email','$message_type_id','$send_type','$title','$message_color_id')");
	$insid=mysql_insert_id();
	}
	if($insid)
	  header("location:index.php?file=me-memories&msg=$msg&stype=$saveType#page=page-2");
	?>
	...
	if($_POST['saveForm']=="Delete")
{
  ...
  header("location:index.php?file=me-memories&stype=$stype&msg=$msg#page=page-2");
}
...
$my_res1=mysql_query("select a.*,b.message_intensity as color_id from messagetrigger a,message_intensity b where a.message_color_id=b.id and message_type_id='4' and email='$tmail' and send_type=0  $msql $dsql and status!='Deleted' and status!='RDeleted'");
}else{
...
$my_res1=mysql_query("select a.*,b.message_intensity as color_id from messagetrigger a,message_intensity b where a.message_color_id=b.id and  message_type_id='4' and sender_email='$tmail' and send_type=0 $msql $dsql and status!='Deleted' and status!='SDeleted'");
}
/*paging statrs */
...
$reload = $_SERVER['PHP_SELF'] . "?file=me-memories&stype=".$stype."&tpages=" . $tpages . "&amp;adjacents=" . $adjacents;
...
$my_res=mysql_query("select a.*,b.message_intensity as color_id from messagetrigger a,message_intensity b where a.message_color_id=b.id and  message_type_id='4' and sender_email='$tmail' and send_type=0 $msql $dsql and status!='Deleted' and status!='SDeleted' order by $order $ot LIMIT $start,$limit");
//}
...
$res.='	<span style="float:left;margin:0px 0px 5px 0px; padding:0px; width:150px;">
 ...
 $res.='<img src="{IMG_URL}attachemtn.gif" width="16" height="14" alt="attachemet" />';
  ...
  $res.='<a href="index.php?file=me-open_memories&stype='.$send_type.'&id='.$id.'">'.$message.'</a>';
...
<a href="#" onclick="streamPublish(\''.$my_arr['title'].'\',\''.$message.'....\',\'\');" ><img src="{IMG_URL}facebook_icon.gif" width="16" height="16" border="0"/></a>
  ...
  <a href="https://twitter.com/share" target="_blank"  data-url="http://emirror.vdevserver.co.cc/" data-text="'.$my_arr['title'].'" data-count="none"><img src="{IMG_URL}twitter_icon.gif" width="16" height="16" /></a><script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src="//platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");</script>
  <a href="#" onclick="return checkmemoryDelete('.$id.')";><img src="{IMG_URL}delete.gif" width="13" flot="right"height="13" /></a></div>
</div>';
...
//echo $order;
$res1='<a href="index.php?file=me-memories&stype='.$send_type.'#page=page-2">All</a>&nbsp;&nbsp;&nbsp;';
$res1.= $pagerecords.paginate_one($reload,$page,$tpages);
if($order=="message" && $otype=="a")
{
$tp=str_replace("{MSG_SORT}",'<a href="index.php?file=me-memories&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order=message&otype=d#page=page-2" >
<img src="{IMG_URL}up_arrow.png" width="12" height="6" alt="descendingorder" title="Descending Sort" align="absmiddle"  border="0" /></a>',$tp);
}
else
{
$tp=str_replace("{MSG_SORT}",'<a href="index.php?file=me-memories&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order=message&otype=a#page=page-2" >
...
{
$tp=str_replace("{SENDER_SORT}",'<a href="index.php?file=me-memories&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order='.$osender.'&otype=d#page=page-2" >
<img src="{IMG_URL}up_arrow.png" width="12" height="6" alt="descendingorder" title="Descending Sort" align="absmiddle"  border="0" /></a>',$tp);
}
else
{
$tp=str_replace("{SENDER_SORT}",'<a href="index.php?file=me-memories&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order='.$osender.'&otype=a#page=page-2" >
<img src="{IMG_URL}down_arrow.png" width="12" height="6" alt="ascendingorder" title="Ascending Sort" align="absmiddle"  border="0" /></a>',$tp);
}
if($order=="create_time" && $otype=="a")
{
$tp=str_replace("{TIME_SORT}",'<a href="index.php?file=me-memories&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order=create_time&otype=d#page=page-2" >
<img src="{IMG_URL}up_arrow.png" width="12" height="6" alt="descendingorder" title="Descending Sort" align="absmiddle"  border="0" /></a>',$tp);
}
else
{
$tp=str_replace("{TIME_SORT}",'<a href="index.php?file=me-memories&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order=create_time&otype=a#page=page-2" >
<img src="{IMG_URL}down_arrow.png" width="12" height="6" alt="ascendingorder" title="Ascending Sort" align="absmiddle"  border="0" /></a>',$tp);
}
if($order=="message_intensity" && $otype=="a")
{
$tp=str_replace("{INTENSITY_SORT}",'<a href="index.php?file=me-memories&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order=message_intensity&otype=d#page=page-2" >
<img src="{IMG_URL}up_arrow.png" width="12" height="6" alt="descendingorder" title="Descending Sort" align="absmiddle"  border="0" /></a>',$tp);
}
else
{
$tp=str_replace("{INTENSITY_SORT}",'<a href="index.php?file=me-memories&stype='.$send_type.'&keyword='.$keyword.'&fromdate='.$fromdate.'&todate='.$todate.'&order=message_intensity&otype=a#page=page-2" >
<img src="{IMG_URL}down_arrow.png" width="12" height="6" alt="ascendingorder" title="Ascending Sort" align="absmiddle"  border="0" /></a>',$tp);
}
...
<input id="saveForm" onclick="return valid_memoryform(document.messageform,\''.$str_words.'\');" name="saveForm" class="inr_btn" type="submit" value="Save"/>
   ',$tp);
if($my_arrdraft['id']!=""){
...
//memories messages
$tp=str_replace("{title}",$title_memory,$tp);
